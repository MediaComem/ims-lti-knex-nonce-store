...
class KnexNonceStore extends NonceStore {
   ...
   * This duration defaults to 5 minutes (300 sec). Pass a value to the `expireIn` argument to change this.
   *
   * A request will only be valid if it's timestamp is "fresh", that is it represents a timestamp not too far in the past.
   * This validity duration defaults to 5 minutes (300 sec), but you can pass a value to the `lifetime` argument to change it.
   ...
   * @param {number} [expireIn=300] - The number of seconds during which the nonce won't be reusable. Defaults to 5 minutes (300 sec).
   * @param {number} [lifetime=300] - The delay (in seconds) during which a timestamp is valid. Defaults to 5 minutes (300 sec).
   ...
   */
  constructor(knex, tableName = 'nonce_store', expireIn = 5 * 60, lifetime = 5 * 60) {
